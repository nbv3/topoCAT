...
/**
 * Face implementation class FaceZp
 * 
 ...
 */
public class FaceZp extends Face {
	...
	public FaceZp( final EdgeI _currentEdge, 
	...
	public FaceZp( final int _vertexIndex1, 
	...
	// 
	public FaceZp( final FaceI _orig ) {
		    ...
		    if ( _dagListToMerge != null && _dagListToMerge.size() > 0 ) {
		    	...
		    	for( SortedMap.Entry<Integer,Integer> dagListItem : _dagListToMerge.entrySet()) {
	    			...
	    			if ( dag2List_.containsKey( key ) ) {
	    				...
	    				prevValue = dag2List_.get( key );
	    				dag2List_.remove( key );
	    				newValue = prevValue + value;
	    				if ( newValue != 0 ) {
    					
	    					dag2List_.put( key, newValue );
	    				}
		    		}
		    		else {
